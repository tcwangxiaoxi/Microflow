...
function removeAttrByHiddenPaths(jsonObj, paths) {
    ...
    for (var i in paths) {
        ...
        // 移除对应层级下的属性
        function deleteAttr(delPaths, obj) {
            var curPath = delPaths[0];
            var restPaths = delPaths.slice(1);
    ...
    for (var i in paths) {
        ...
        function deleteAttr(paths, obj) {
            ...
            if (restPaths.length > 0) {
                ...
                var showItems = [curPath];
                // 如果现实多个字段，需要如下格式：[a|b|c]
                if (curPath.indexOf('[') == 0) {
                    var ns = curPath.substr(1, curPath.length - 2);
                    showItems = ns.split('|');
                }
                // 已经是最后一层了，删除除了指定属性外的其他所有属性
                for (var attr in obj) {
                    var needshow = false;
                    for (var itemsIndex in showItems) {
                        var item = showItems[itemsIndex];
                        // 如果找到了匹配的则标记为需要显示，并遍历下面的元素
                        if (attr == item) {
                            needshow = true;
                            continue;
                        }
